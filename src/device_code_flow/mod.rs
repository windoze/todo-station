    ...
    T: Into<Cow<'a, str>>,
{
    ...
    let err = HttpError::new(rsp).await;
    if !err.status().is_success() {
        return Err(
            ErrorKind::http_response(err.status(), err.error_code().map(String::from)).into_error(),
        );
    }
    let device_code_response: DeviceCodePhaseOneResponse = from_json(err.body())?;
